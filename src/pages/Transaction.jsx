  ...
  // ▼▼▼ CORREÇÃO: Importando os componentes do Dialog que vamos usar ▼▼▼
  ...
  const Transactions = () => {
    ...
    // --- NOVO ESTADO PARA O MODAL DE CONFIRMAÇÃO DE IMPRESSÃO ---
    ...
    }, [showNewSale, currentProductPage, productSearchTerm, fetchProducts, fetchPaymentMethods]);
    ...
    const handleAddPayment = () => {
      ...
      setPayments([...payments, { payment_method_id: selectedMethod.id, name: selectedMethod.name, amount }]);
    ...
    // ▼▼▼ FUNÇÃO ATUALIZADA PARA ABRIR O MODAL DE CONFIRMAÇÃO ▼▼▼
    const handleFinalizeSale = async () => {
      ...
      try {
        ...
        // Guarda a transação finalizada e abre o modal de confirmação
        setLastTransaction(finalizedTransaction);
        setShowPrintConfirm(true);
        
        // Limpa o estado do PDV, mas NÃO o da página inteira ainda
        setCart([]);
        setDiscountPercent(0);
        setPayments([]);
        setCurrentPaymentAmount('');
    ...
    // ▼▼▼ NOVAS FUNÇÕES PARA CONTROLAR O MODAL DE IMPRESSÃO ▼▼▼
    const handlePrintConfirm = async () => {
      ...
      try {
        ...
        fetchTransactions(1); // Atualiza a lista de vendas
      }
    };

    const handlePrintCancel = () => {
      // Apenas fecha o modal e reseta a página
      setShowPrintConfirm(false);
      setLastTransaction(null);
      fetchTransactions(1); // Atualiza a lista de vendas
    };
    ...
    const getPaymentMethodIcon = (methodName) => {
      ...
      if (name.includes('crédito')) return <CreditCard className="w-4 h-4" />;
    ...
    return (
      <div className="p-6 bg-gray-50 min-h-screen">
        ...
        {/* Modal de Nova Venda (PDV) - Sem alterações */}
        {showNewSale && (
        ...
        <Dialog open={showPrintConfirm} onOpenChange={setShowPrintConfirm}>
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              ...
              <DialogDescription>
                A venda #{lastTransaction?.sale_number} foi finalizada. O que você deseja fazer?
              </DialogDescription>
            </DialogHeader>
            <DialogFooter className="sm:justify-between gap-2 pt-4">
              <Button type="button" variant="secondary" onClick={handlePrintCancel}>
                Finalizar (Não Imprimir)
              </Button>
        ...
        {/* Dashboard e Histórico de Vendas - Sem alterações */}
