...
const Notifications = () => {
  ...
  const fetchAlerts = async () => {
    try {
      ...
      const allAlerts = [
        ...(receivablesRes.data.accounts || []),
        ...(payablesRes.data.accounts || []),
      ];
  ...
  return (
    <Popover>
      {/* CORREÇÃO DO ERRO: usar asChild aqui */}
      <PopoverTrigger asChild>
        <Button variant="ghost" className="relative h-12 w-12 rounded-full">
          <Bell size={50} className="!h-6 !w-6" />
      ...
      <PopoverContent className="w-80 p-0">
        <div className="p-4">
          ...
          <p className="text-sm text-muted-foreground">
            {totalAlerts > 0
              ? `Você tem ${totalAlerts} contas vencidas ou próximas do vencimento.`
          ...
          {loading ? (
            ...
            alerts.map(alert => (
              <Link
                ...
                className="block border-t p-4 hover:bg-gray-50"
              >
