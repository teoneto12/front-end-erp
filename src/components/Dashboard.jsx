...
const Dashboard = ({ transactions }) => {
  // Se não houver transações, não renderiza o dashboard.
  ...
  const totalItemsSold = activeTransactions.flatMap(t => t.items || []).reduce((acc, item) => acc + (item.quantity || 0), 0);
  ...
  const salesOverTime = activeTransactions.reduce((acc, t) => {
    ...
    acc[date] += parseFloat(t.total_amount || 0);
    return acc;
  }, {});
  ...
  const topProducts = activeTransactions
    ...
    .reduce((acc, item) => {
      ...
      acc[productName] += (item.quantity || 0);
      return acc;
    }, {});
